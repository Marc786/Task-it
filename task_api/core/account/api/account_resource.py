...
from task_api.core.account.api.config.account_dependency_factory import create_account_service
from task_api.core.account.api.dto.request.create_account_request import CreateAccountRequest
...
def get_account(
    ...
    account_response_assembler: AccountResponseAssembler = Depends(AccountResponseAssembler),
):
    accounts = account_service.get_account()
    accounts_response = account_response_assembler.accounts_to_response(accounts)

    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content=[account_response.model_dump() for account_response in accounts_response],
    )
